...
def puissance_matrice_2_2 (M, n) :
  ...
  else : return produit_matrice_2_2(M, carre)
...
def puissance_matrice_2_2_ops (M, n) :
  ...
  ops += 1
  carre, ops_tmp = produit_matrice_2_2_ops(tmp, tmp)
  ops += ops_tmp
  ops += 1
  if n % 2 == 0 : return carre, ops
  else : tmp, ops_tmp = produit_matrice_2_2_ops(M, carre)
  return tmp, ops + ops_tmp
...
def produit_matrice_2_2_bits (M1, M2, compte = decompte_naif) :
  ...
  res[0][0] = M1[0][0] * M2[0][0] + M1[0][1] * M2[1][0]
  bits += compte(res[0][0], M1[0][0], M2[0][0], M1[0][1], M2[1][0])
  res[1][0] = M1[1][0] * M2[0][0] + M1[1][1] * M2[1][0]
  bits += compte(res[1][0], M1[1][0], M2[0][0], M1[1][1], M2[1][0])
  res[1][0] = M1[1][0] * M2[0][0] + M1[1][1] * M2[1][0]
  bits += compte(res[1][0], M1[1][0], M2[0][0], M1[1][1], M2[1][0])
  res[1][0] = M1[1][0] * M2[0][0] + M1[1][1] * M2[1][0]
  bits += compte(res[1][0], M1[1][0], M2[0][0], M1[1][1], M2[1][0])
...
def puissance_matrice_2_2_bits (M, n, compte = decompte_naif) :
  ...
  bits = 0
  if n == 1 : return M, bits
  tmp, bits_tmp = puissance_matrice_2_2_bits(M, n // 2)
  bits += bits_tmp
  carre, bits_tmp = produit_matrice_2_2_bits(tmp, tmp)
  bits += bits_tmp
  if n % 2 == 0 : return carre, bits
  else : return produit_matrice_2_2(M, carre), bits
